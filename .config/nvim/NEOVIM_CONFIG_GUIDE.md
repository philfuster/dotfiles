...
This document provides a comprehensive overview of this Neovim configuration,
which is built on top of [LazyVim](https://github.com/LazyVim/LazyVim) with
...
├── CLAUDE.md                 # Conventions and gotchas for AI agents
├── NEOVIM_CONFIG_GUIDE.md    # This file - feature and plugin tour
├── CEPs/                     # Config Enhancement Proposals
├── lua/
...
This configuration uses **Prettier** (via Mason and conform.nvim) for formatting
markdown files:

- Prettier is installed and managed by Mason (`:Mason` in Neovim).
- Prettier only runs when it finds a config file for the buffer, because
...
- `lua/plugins/extend-conform.lua` sets no config path. It declares only the
  formatter order per filetype, so Biome takes over when no Prettier config is
  found:
  ...
  This replaced 85 lines of hand-rolled config detection - see
  [CEP-005](CEPs/CEP-005-configuration-cleanup.md).

- Prettier locates its config by searching upward from the file being formatted.
  Files in this repository resolve to `~/.config/.prettierrc`, which sets
  `printWidth: 80` and `proseWrap: always` for markdown:
...
- Markdown linting is provided by `markdownlint-cli2`, also managed by Mason.
  Diagnostics appear as virtual text or in the diagnostics panel.

**Troubleshooting:**

- If formatting does not wrap lines, run `:ConformInfo` to confirm Prettier
  actually ran and that a config file was found.
- If Biome formats a file you expected Prettier to handle, no Prettier config
  was found above it. Add one, or unset `vim.g.lazyvim_prettier_needs_config`.

**Key commands:**

- Format: `<leader>cf` or `:lua require("conform").format()`
- Lint: Diagnostics appear automatically; use `:Trouble` or
  `:lua vim.diagnostic.open_float()` to view.

## 🛠 Base Configuration (LazyVim)

This configuration is built on **LazyVim**, a modern Neovim configuration
...
_See the actual file for specific key bindings and implementation details._
...
- **Quick Navigation**: Press `q` to close help, quickfix, and other special
  buffers

_Implementation details available in the configuration file._
...
**Purpose**: Seamless window navigation using Alt+hjkl, with intelligent
detection of terminal multiplexers like tmux.

#### nvim-spider (`nvim-spider.lua`)

Smarter word motion that skips punctuation.

**Purpose**: Enhanced w/e/b motions that intelligently handle punctuation and
...
**Purpose**: Modern search/replace interface with visual feedback (triggered
with `g/`).
...
**Purpose**: Convert text between camelCase, snake_case, UPPER_CASE, etc. with
interactive picker (`ga.`).

#### Better Escape (`better-escape.lua`)

Enhanced escape sequences for smoother editing.

**Purpose**: Multiple escape options (jj, jk) with optimized timing to reduce
conflicts.

### Enhanced User Experience

#### Todo Management (`dooing.lua`)

Global and per-project todo lists with due dates and nested tasks.

**Purpose**: One global list kept in `~/.config/dooing_todos.json` so it
survives an `~/.local/share/nvim` wipe, plus opt-in per-project `dooing.json`
lists that are only created after a prompt. Due and overdue items are surfaced
on startup. The three entry points live under the `<leader>k` group
(`<leader>kk` global, `<leader>kp` project, `<leader>kd` due) so that dooing's
`<leader>t` defaults do not shadow LazyVim's test group. Every other binding is
buffer-local to the todo window - press `?` inside the list for the in-window
keys (`i` add, `x` toggle, `d` delete, `q` close, `<leader>kn` nested sub-task).

#### Todo Comments (`todo-comments.lua`)

Highlight and navigate TODO comments with style.

**Purpose**: Beautiful highlighting of TODO, FIXME, HACK, etc. with navigation
(`]t`/`[t`) and search integration.

#### Better Quickfix (`nvim-bqf.lua`)

Enhanced quickfix window experience with better navigation and preview.

**Purpose**: Improved quickfix list with preview capabilities and better
interaction.
...
**Purpose**: Streamlined conflict resolution with clear visual markers and
shortcuts for choosing changes. Navigate between conflicts and manage resolution
efficiently.
...
**Purpose**: Smart indentation detection that overrides editorconfig when
needed.
...
Enhanced mini-files explorer with convenient access to different directory
contexts.

**Purpose**: Quick file navigation with shortcuts for current file directory,
...
**Purpose**: Quick access to previous sessions directly from the dashboard for
...
Rather than duplicating all key bindings in this guide, use these methods to
discover them:
...
- Use the keymaps picker (`<leader>sk`) → Search through all key mappings
...
4. **Search & Development**
   ...
   - Global and per-project todo lists with due dates (`<leader>k`)
...
4. Use the full `"author/repository"` format - see [CLAUDE.md](CLAUDE.md) for
   the plugin naming standard
5. Add a short section for it under [🔌 Custom Plugins](#-custom-plugins) in
   this guide
...
- **Disabled Plugins**: Removed unnecessary default plugins (gzip, tar, zip,
  etc.)
...
The `lazy-lock.json` file locks plugin versions for reproducibility. Commit this
file when you want to lock the current plugin versions.

### Configuration Backup

This configuration is managed through a bare git repository (see
[../DOTFILES_MANAGEMENT.md](../DOTFILES_MANAGEMENT.md), which lives in
`~/.config`):
...
- **Change Proposals**: [CEPs/README.md](CEPs/README.md)
...
- **Better Options**: Added performance tweaks, scroll offsets, smart search,
  and persistent undo
- **Enhanced Keymaps**: Added text movement, better indenting, centered
  navigation, and quick save
- **Smart Autocmds**: Automatic yank highlighting, directory creation, and quick
  close for special buffers

### New Plugin Additions

- **better-escape.nvim**: Multiple escape sequences (`jj`, `jk`) with optimized
  timing
- **dooing**: Global and per-project todo lists with due dates and nested tasks,
  under the `<leader>k` group
- **todo-comments.nvim**: Beautiful TODO highlighting with navigation and picker
  integration
- **nvim-bqf**: Enhanced quickfix window experience

### Plugin Improvements

- **text-case.nvim**: Added Telescope integration for interactive case
  conversion
...
- **Visual Feedback**: Highlight yanked text for better copy/paste awareness
- **Smart Directory Creation**: Automatically create parent directories when
  saving files
...
_This configuration prioritizes developer productivity, performance, and
maintainability while providing modern IDE-like features in Neovim._
